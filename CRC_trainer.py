...
from datasets.dataset_CRC import CRC_dataset, RandomGenerator, RandomGenerator_test
...
def trainer(args, model):
    ...
    if args.dataset == datafile_name:
        db_train = CRC_dataset(base_dir=args.train_path, list_dir='./lists/lists_CRC', split="train",
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size])]))
        db_validate = CRC_dataset(base_dir='data/'+datafile_name+'/test_vol/', list_dir='./lists/lists_CRC', split="test_vol",
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size])]))
    ...
    optimizer = optim.AdamW(model.parameters(), betas=(0.9, 0.999), eps=1e-08)
